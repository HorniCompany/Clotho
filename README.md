...
The defining characteristic of the **Clotho Framework** is its absolute rejection of high-level API wrappers (like Mineflayer). We are building a low-level, high-performance core (C++ / TypeScript) that interacts with any environment exactly as a human does:
...
This architectural shift transforms Oneiro from a mere "game bot" into a true **General Embodied Agent**. Whether it's Minecraft, a terminal, or a 3D engine, Clotho plays it by looking at the screen and pressing buttons.
...
To achieve both high-frequency reaction times and complex long-term reasoning, Clotho splits cognitive load via a **Mixture of Agents (MoA)**:
...
To comply with strict data localization and privacy laws (e.g., Federal Law No. 152-FZ regarding biometric data), Clotho **does not use voice biometrics** to recognize who is speaking.
...
Inherited and enhanced from the base Hermes architecture, Clotho features a persistent **Closed Learning Loop**:
...
*(Note: **Clotho** is the underlying engine. **Oneiro** is the specific neuro-product / AI personality that runs on top of this framework in the Horni universe).*
